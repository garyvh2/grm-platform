...
const feature = loadFeature(resolve(__dirname, '../features/music-contracts.feature'));
...
defineFeature(feature, (test) => {
  ...
  const givenSuppliedReferenceData = (given: StepFn) => {
    given('the supplied reference data', () => {
      const musicText = readFileSync(
        resolve(__dirname, '../../public/data/music-contracts.txt'),
        'utf-8',
      );
      const partnerText = readFileSync(
        resolve(__dirname, '../../public/data/partner-contracts.txt'),
        'utf-8',
      );
  ...
  test('Search for active music contracts', ({ given, when, then }) => {
    givenSuppliedReferenceData(given);
    whenUserPerformSearch(when);
    thenOutputShouldBe(then);
  });

  test('Search for active music contracts_2', ({ given, when, then }) => {
    givenSuppliedReferenceData(given);
    whenUserPerformSearch(when);
    thenOutputShouldBe(then);
  });

  test('Search for active music contracts_3', ({ given, when, then }) => {
    givenSuppliedReferenceData(given);
    whenUserPerformSearch(when);
    thenOutputShouldBe(then);
  });
